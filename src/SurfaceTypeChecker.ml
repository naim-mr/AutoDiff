...
let rec print_type ty =
  ...
  | TUnknown -> "unknown"
  | TReal -> "real"
  | TTuple tys -> sprintf "[%s]" (String.concat ", " (List.map print_type tys))
...
let raise_failure msg = raise (Failure msg)
let fail_raw format = ksprintf raise_failure format

let fail range format =
  ksprintf
    (fun msg -> fail_raw "%s%s\n" (MenhirLib.LexerUtil.range range) msg)
    format

let on_failure handle f = try f () with Failure msg -> handle msg
...
let rec known range ty =
  match ty with
  | TUnknown -> fail range "An unknown type appears in a type annotation."
  | TReal -> ()
  | TTuple tys -> iter (known range) tys
...
let defensive_lookup range kind env x =
  try Env.lookup env x with Not_found -> fail range "Unbound %s: %s." kind x
...
let check_distinct range kind xs =
  ...
  with Env.Collision x -> fail range "Multiply-defined %s: %s." kind x
...
let rec infer range (fenv : fenv) (env : env) (e : expr) : ty =
  match e with
  | Loc (e, range) -> infer range fenv env e
  | Var x -> defensive_lookup range "variable" env x
  | Literal _ -> TReal
  | UnOp (_uop, e) ->
  ...
  | TupleIntro es -> TTuple (List.map (infer range fenv env) es)
  | TupleElim (xs, e1, e2) ->
  ...
  | FunCall (f, es) ->
      ...
      if length tys <> length es then
        fail range "Expected %d arguments; got %d arguments." (length tys)
          (length es);
...
and check_real range fenv env e =
  ...
  | TReal -> ()
  | TTuple tys ->
      fail range "Expected a real; got a tuple of arity %d." (length tys)

and check_tuple range fenv env e : tys =
  match infer range fenv env e with
  | TUnknown -> assert false
  | TReal -> fail range "Expected a tuple; got a real."
  | TTuple tys -> tys

and check_expected range fenv env e (expected : ty) =
  let actual = infer range fenv env e in
  if expected <> actual then
    fail range "Expected type %s; got type %s." (print_type expected)
      (print_type actual)
...
let infer_decls fenv decls : fenv = List.fold_left infer_decl fenv decls
...
let check_expr fenv e expected =
  ...
  and handle msg =
    fail_raw
      "The expression %s either is ill-typed\n\
