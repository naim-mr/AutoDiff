...
type command = Undefined | ReadFile of (* filename: *) string | Random

let command = ref Undefined

type processing = ForwardModeOnly | ForwardModeAndReverseMode

let processing = ref ForwardModeAndReverseMode
let budget = ref 5
let mark = ref false
let usage = sprintf "Usage: %s <options> <filename>\n" Sys.argv.(0)
let resilient = ref false
let show_surface = ref false
let show_linear = ref false
let test_forward, test_unzip, test_reverse = (ref false, ref false, ref false)
let verbose = ref false
let highlight_style = ref LinearPrinter.Ansi
...
let highlight_styles_descr =
  Printf.sprintf "[ %s ]" (String.concat " | " (List.map fst highlight_styles))
...
let set_highlight_style str =
  ...
  | exception _ ->
      Printf.ksprintf
        ...
        "--highlight-style <style>: use one of %s" highlight_styles_descr
  ...
  Arg.parse
    (Arg.align
       [
         ( "--budget",
           Arg.Set_int budget,
           "<int> sets a random generation budget" );
         ( "--forward-mode",
           Arg.Unit (fun () -> processing := ForwardModeOnly),
         ...
         ( "--reverse-mode",
           Arg.Unit (fun () -> processing := ForwardModeAndReverseMode),
         ...
         ( "--test-forward",
           Arg.Set test_forward,
           " test forward mode on the fly" );
         ("--test-unzip", Arg.Set test_unzip, " test unzipping on the fly");
         ( "--test-reverse",
           Arg.Set test_reverse,
           " test reverse mode on the fly" );
         ( "--highlight-style",
           Arg.String set_highlight_style,
           "<style> choose error highlight style among "
           ^ highlight_styles_descr );
         ("--verbose", Arg.Set verbose, " show progress messages");
       ])
    (fun name -> command := ReadFile name)
    usage
...
let command =
  match !command with Undefined | Random -> Random | ReadFile _ -> !command

let mark = !mark
let processing = !processing
let resilient = !resilient
let show_surface = !show_surface
let show_linear = !show_linear
...
let read filename : Surface.prog =
  ...
  try SurfaceParser.prog SurfaceLexer.token lexbuf
...
module S = struct
  ...
  let check olabel (prog : prog) : prog =
    ...
    | _fenv ->
        ...
        prog
    | exception Failure msg -> maybe_exit msg

  let test_reverse_mode prog prog' =
    (if test_reverse then
     try Test.test_reverse_mode prog prog'
     with Test.TestFailure msg -> maybe_exit msg);
    prog'

  let print prog' =
    match command with ReadFile _ -> print (print_program prog') | _ -> ()
end
...
module L = struct
  ...
  let check olabel mode (prog : prog) : prog =
    ...
    | exception LinearTypeChecker.Error (range, msg) ->
        (* If the program is ill-typed, then we show the program *before*
           type-checking. *)
        let ranges =
          Option.fold ~none:RangeSet.empty ~some:RangeSet.singleton range
        in
        let highlight = { ranges; style = !highlight_style } in
  ...
  let progress olabel msg mode prog = prog |> info msg |> check olabel mode

  let simplify olabel prog =
    prog |> Freshen.transform
  ...
  let resurface_silently (prog : Linear.prog) : Surface.prog =
    prog |> Forget.transform |> Simplify.transform |> Linear2Surface.translate
    |> Compress.transform
    |> S.check (Some "FMAD_RESURFACED")

  let resurface (prog : Linear.prog) : Surface.prog =
    prog |> Forget.transform |> Simplify.transform |> Linear2Surface.translate
    |> Compress.transform |> info "Back up to Surface."
    |> S.check (Some "RMAD_RESURFACED")

  let test_forward_mode prog prog' =
    (if test_forward then
     try Test.test_forward_mode prog (resurface_silently prog')
     with Test.TestFailure msg -> maybe_exit msg);
    prog'

  let test_unzip fs prog prog' =
    (if test_unzip then
     try Test.test_unzip fs prog prog'
     with Test.TestFailure msg -> maybe_exit msg);
    prog'
end
...
let half2 (sprog : Surface.prog) (prog : Linear.prog) : Surface.prog =
  ...
  | ForwardModeOnly -> L.resurface prog
  | ForwardModeAndReverseMode -> half2 sprog prog

(* The complete pipeline. *)

let pipeline (sprog : Surface.prog) =
  sprog |> half1 |> half2 sprog
...
let attempts budget =
  if budget < 5 then 1000
  else if budget < 10 then 500
  else if budget < 30 then 100
  else if budget < 60 then 50
  else 10
  ...
  | Random ->
      ...
      for budget = 0 to budget do
        ...
        for _ = 1 to attempts do
          try SurfaceGenerator.generate budget |> pipeline with Abandon -> ()
        done;
