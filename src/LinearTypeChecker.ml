...
let fail format = ksprintf (fun msg -> raise (Error (None, msg))) format
...
let with_range range f x =
  try f x with Error (None, msg) -> raise (Error (Some range, msg))
...
let rec known (ty : ty) : unit =
  match ty with
  | TUnknown -> fail "The type `unknown` appears in a type annotation"
  | TReal -> ()
  | TTuple tys -> iter known tys
...
let arity_conforms (expected : mvty) (inferred : mvty) =
  ...
  if uexpected <> uinferred then
    fail "Expected %d unrestricted wires; got %d unrestricted wires" uexpected
      uinferred;
  if lexpected <> linferred then
    fail "Expected %d linear wires; got %d linear wires" lexpected linferred
...
let mvconforms (mvty1 : mvty) (mvty2 : mvty) : unit =
  ...
  let utys1, ltys1 = mvty1 and utys2, ltys2 = mvty2 in
...
let untuple (ty : ty) : tys =
  match ty with TTuple tys -> tys | TUnknown | TReal -> assert false
...
let lookup kind env x =
  try Env.lookup env x with Not_found -> fail "Unbound %s: %s" kind x
...
let check_distinct kind xs =
  try Env.check_distinct xs
  with Env.Collision x -> fail "Multiply-defined %s: %s" kind x
...
let flookup fenv f = lookup "function" fenv f

(* -------------------------------------------------------------------------- *)

(* Environments for unrestricted variables. *)

type uenv = ty Env.env

let uproj (U x) = x
let ulookup uenv x = lookup "unrestricted variable" uenv (uproj x)
...
let ubindb2_many uenv (bs : ubindings) (inferred : tys) : uenv * ubindings =
  ...
  let tys = inferred in
  let uenv = ubind_many uenv xs tys and bs = zip xs tys in
  (uenv, bs)
...
let lproj (L x) = x
let llookup lenv x = lookup "linear variable" lenv (lproj x)
let lbind_many lenv xs tys = bind_many "linear variable" lenv (map lproj xs) tys
...
let lbindb2_many lenv (bs : lbindings) (inferred : tys) : lenv * lbindings =
  ...
  iter2 conforms declared inferred;
  let tys = inferred in
  let lenv = lbind_many lenv xs tys and bs = zip xs tys in
  (lenv, bs)
...
let bindb2_many (env : env) (bs : bindings) (inferred : mvty) : env * bindings =
  let uenv, lenv = env and ubs, lbs = bs and uinferred, linferred = inferred in
  ...
  let env = (uenv, lenv) and bs = (ubs, lbs) in
  (env, bs)
...
let rec infer (fenv : fenv) (env : env) (e : expr) : mvty * expr =
  match e with
  | Loc (e, range) -> with_range range (infer fenv env) e
  | Ret (uxs, lxs) ->
      let uenv, lenv = env in
      ((map (ulookup uenv) uxs, map (llookup lenv) lxs), e)
  | Let (ubs, lbs, e1, e2) ->
      let bs = (ubs, lbs) in
      ...
      let ubs, lbs = bs in
      (mvty2, Let (ubs, lbs, e1, e2))
  | ULiteral _ -> (u1 real, e)
  | UUnOp (_, x) ->
      let uenv, _ = env in
  ...
  | LAdd (x1, x2) ->
      ...
      let _, lenv = env in
      let ty1 = llookup lenv x1 and ty2 = llookup lenv x2 in
      conforms ty1 ty2;
      (* an equality check *)
      (l1 ty1, e)
  | LMul (x1, x2) ->
...
let check_decl fenv decl : fenv * decl =
  ...
  | Decl (range, f, ubs, lbs, e) ->
      ...
      with_range range
        (fun () ->
          let uenv = ubindb1_many empty ubs and lenv = lbindb1_many empty lbs in
          let env = (uenv, lenv) in
          let codomain, e = infer fenv env e in
          let fty = (domain, codomain) in
          let fenv = fbind fenv f fty in
          (fenv, Decl (range, f, ubs, lbs, e)))
        ()

let check_decls = List.fold_left_map check_decl
let name_of decl = match decl with Decl (_, f, _, _, _) -> f
...
include struct
  ...
  let add (x : lvar) (xs : t) : t = if mem x xs then used_twice x else add x xs
  let of_list (xs : lvars) : t = List.fold_right add xs empty
  ...
  let rec flv (e : expr) : t =
    match e with
    | Loc (e, range) -> with_range range flv e
    | UTupleElim (_, _, e) -> flv e
  ...
  let check_linearity decls = List.iter check_linearity decls
end
