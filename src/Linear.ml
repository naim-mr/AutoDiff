...
type ty = Surface.ty = TUnknown | TReal | TTuple of tys
and tys = ty list
...
let ufrag ((tys, _) : mvty) : tys = tys
let lfrag ((_, tys) : mvty) : tys = tys
...
let domain ((ty, _) : fty) : mvty = ty
let codomain ((_, ty) : fty) : mvty = ty
...
type uvar = U of string [@@unboxed]
type uvars = uvar list

(* Linear variables. *)

type lvar = L of string [@@unboxed]
...
let fbind (fenv : fenv) (f : name) (fty : fty) : fenv = Env.bind fenv f fty

(* -------------------------------------------------------------------------- *)

(* A binding is a pair of a variable and a type. *)

type ubinding = uvar * ty
type ubindings = ubinding list
type lbinding = lvar * ty
type lbindings = lbinding list
type bindings = ubindings * lbindings

(* -------------------------------------------------------------------------- *)

(* Unary operators. *)

type unop = Surface.unop = OpSin | OpCos | OpExp

(* Binary operators. *)

type binop = Surface.binop = OpAdd | OpSub | OpMul | OpDiv

(* -------------------------------------------------------------------------- *)

(* Source code ranges. *)

type range = Source of Surface.range | Virtual of int
...
type decl = Decl of range * name * ubindings * lbindings * expr
...
let name (decl : decl) : name = match decl with Decl (_, f, _, _, _) -> f
let names (decls : decls) : names = List.map name decls


let get_nv = function | U s -> s 