...
let fresh, clear = Gensym.make ()
let fresh _ : var = Printf.sprintf "x%d" (fresh ())
let ffresh, fclear = Gensym.make ()
let ffresh _ : name = Printf.sprintf "f%d" (ffresh ())
...
let split budget = split_max budget budget
...
let literals : expr list = List.init 5 (fun i -> lit (Real.of_int (i - 2)))
(* -2, -1, 0, +1, +2 *)

let literal () = choose literals
  ...
  if Env.is_empty goals then e
    ...
    let possibilities =
      [
        ...
        (true, satisfy_goal_via_literal budget fenv hyps goals e);
        (* Allowed if the budget is nonzero: *)
        (budget > 0, satisfy_goal_via_unary_op budget fenv hyps goals e);
        (* Allowed if the budget is nonzero: *)
        (budget > 0, satisfy_goal_via_binary_op budget fenv hyps goals e);
...
let generate_decl_with_name_and_type budget fenv f fty : decl * fenv =
  ...
  (Decl (f, zip xs tys, e, dummy), Env.bind fenv f fty)
