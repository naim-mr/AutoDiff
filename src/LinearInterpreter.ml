...
let eval_unop, eval_binop = SurfaceInterpreter.(eval_unop, eval_binop)
...
type uenv = value Env.env
type lenv = value Env.env
type env = uenv * lenv
type fenv = decl Env.env

let empty, bind = Env.(empty, bind)
let lookup, bind_many = SurfaceInterpreter.(lookup, bind_many)
...
let range = Surface.dummy
let ulookup uenv (U x) = lookup range "unrestricted variable" uenv x

let ubind_many kind uenv bs vs =
  bind_many range kind uenv (map (fun (U x, _ty) -> x) bs) vs

let llookup lenv (L x) = lookup range "linear variable" lenv x

let lbind_many kind lenv xs vs =
  bind_many range kind lenv (map (fun (L x, _ty) -> x) xs) vs

let bind_many (uenv, lenv) (ubs, lbs) (uvs, lvs) =
  ( ubind_many "an unrestricted part" uenv ubs uvs,
    lbind_many "a linear part" lenv lbs lvs )

let flookup fenv f = lookup range "function" fenv f
let fbind fenv f decl = bind fenv f decl
...
let as_real = as_real range
let as_tuple = as_tuple range
...
let rec zero ty =
  ...
  | TReal -> VReal Real.zero
  | TTuple tys -> VTuple (map zero tys)

(* Polymorphic addition. *)

let rec add v1 v2 =
  match (v1, v2) with
  | VReal v1, VReal v2 -> real Real.(v1 +. v2)
  | VTuple vs1, VTuple vs2 when length vs1 = length vs2 ->
      VTuple (List.map2 add vs1 vs2)
  | _ ->
      SurfaceInterpreter.fail range
        "Invalid arguments in a linear addition: %s +. %s" (print_value v1)
        (print_value v2)
...
let rec eval_expr (fenv : fenv) (env : env) (e : expr) : result =
  match e with
  | Loc (e, _range) -> eval_expr fenv env e
  | Ret (uxs, lxs) ->
  ...
  | UBinOp (x1, bop, x2) ->
      let uenv, _ = env in
      let v1 = ulookup uenv x1 and v2 = ulookup uenv x2 in
      u1 (real (eval_binop bop (as_real v1) (as_real v2)))
  | LZero ty -> l1 (zero ty)
  | LAdd (x1, x2) ->
      let _, lenv = env in
      let v1 = llookup lenv x1 and v2 = llookup lenv x2 in
      l1 (add v1 v2)
  | LMul (x1, x2) ->
      let uenv, lenv = env in
      let v1 = ulookup uenv x1 and v2 = llookup lenv x2 in
...
let record_decls (decls : decls) : fenv = List.fold_left record_decl empty decls
