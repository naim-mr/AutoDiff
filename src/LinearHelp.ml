...
let uvar (x : uvar) : expr = Ret ([ x ], [])
let lvar (x : lvar) : expr = Ret ([], [ x ])

(* Converting variables to bindings, carrying an [unknown] type. *)

let unknown x = (x, TUnknown)
...
module UVarSet = struct
  ...
  let ( + ) = union
  let reduce f xs = List.fold_left (fun s x -> union s (f x)) empty xs
  let remove_ubinding ((x, _) : ubinding) (s : t) : t = remove x s

  let remove_ubindings (ubs : ubindings) (s : t) : t =
    List.fold_right remove_ubinding ubs s

  let pairwise_distinct (xs : uvars) = length xs = cardinal (of_list xs)
end
...
module LVarSet = struct
  ...
  let ( + ) = union
  let reduce f xs = List.fold_left (fun s x -> union s (f x)) empty xs
  let remove_lbinding ((x, _) : lbinding) (s : t) : t = remove x s

  let remove_lbindings (lbs : lbindings) (s : t) : t =
    List.fold_right remove_lbinding lbs s

  let pairwise_distinct (xs : lvars) = length xs = cardinal (of_list xs)
end
...
include struct
  ...
  let rec fuv (e : expr) : t =
    match e with
    | Loc (e, _) | LTupleElim (_, _, e) -> fuv e
    | Ret (uxs, _) | UTupleIntro uxs | FunCall (_, uxs, _) -> of_list uxs
    | Let (ubs, _, e1, e2) -> fuv e1 + remove_ubindings ubs (fuv e2)
    | ULiteral _ | LZero _ | LTupleIntro _ | LAdd _ | Drop _ | Dup _ -> empty
    | UUnOp (_, x) | LMul (x, _) -> singleton x
    | UBinOp (x1, _, x2) -> of_list [ x1; x2 ]
    | UTupleElim (ubs, x, e) -> singleton x + remove_ubindings ubs (fuv e)
end
...
include struct
  ...
  let rec flv (e : expr) : t =
    match e with
    | Loc (e, _) | UTupleElim (_, _, e) -> flv e
    | Ret (_, lxs) | LTupleIntro lxs | FunCall (_, _, lxs) -> of_list lxs
    | Let (_, lbs, e1, e2) -> flv e1 + remove_lbindings lbs (flv e2)
    | ULiteral _ | UUnOp _ | UBinOp _ | LZero _ | UTupleIntro _ -> empty
    | LAdd (x1, x2) -> of_list [ x1; x2 ]
    | LMul (_, x) | Drop x | Dup x -> singleton x
    | LTupleElim (lbs, x, e) -> singleton x + remove_lbindings lbs (flv e)
end
...
let current_namespace = ref None
let fresh_names_in_namespace s = current_namespace := Some s
let no_fresh_names () = current_namespace := None
...
let ufresh, uclear = Gensym.make ()

let ufresh () : uvar =
  U (Printf.sprintf "%su%d" (current_namespace ()) (ufresh ()))

let ufreshb (ty : ty) : ubinding = (ufresh (), ty)
let lfresh, lclear = Gensym.make ()

let lfresh () : lvar =
  L (Printf.sprintf "%sl%d" (current_namespace ()) (lfresh ()))

let lfreshb (ty : ty) : lbinding = (lfresh (), ty)
...
module Constructors = struct
  ...
  let one : expr = ULiteral Real.one
  let minus_one : expr = ULiteral Real.minus_one
  let lit c : expr = ULiteral c
  (* Unrestricted operations. *)

  let cos e = with_uvar e @@ fun x -> UUnOp (OpCos, x)
  let sin e = with_uvar e @@ fun x -> UUnOp (OpSin, x)
  let exp e = with_uvar e @@ fun x -> UUnOp (OpExp, x)

  let ( * ) e1 e2 =
    with_uvar e1 @@ fun x1 ->
    with_uvar e2 @@ fun x2 -> UBinOp (x1, OpMul, x2)

  let ( / ) e1 e2 =
    with_uvar e1 @@ fun x1 ->
    with_uvar e2 @@ fun x2 -> UBinOp (x1, OpDiv, x2)

  let uminus e = minus_one * e

  (* Linear operations. *)

  let ( +. ) e1 e2 =
    with_lvar e1 @@ fun dx1 ->
    with_lvar e2 @@ fun dx2 -> LAdd (dx1, dx2)

  let ( *. ) e1 e2 =
    with_uvar e1 @@ fun x1 ->
    with_lvar e2 @@ fun dx2 -> LMul (x1, dx2)

  let lminus e = minus_one *. e
  let ( -. ) e1 e2 = e1 +. lminus e2
  ...
  let pair (e1 : expr) (e2 : expr) =
    with_uvar e1 @@ fun x1 ->
    with_lvar e2 @@ fun dx2 -> Ret ([ x1 ], [ dx2 ])

  (**[uwiden e extra] extends the box [e] with the unrestricted output wires
     represented by th  e variables [extra]. The expression [e] is currently
  ...
  let rec uwiden (e : expr) (extra : uvars) =
    match (e, extra) with
    | _, [] -> e
    | Ret (uxs, lxs), _ -> Ret (uxs @ extra, lxs)
    | (ULiteral _ | UUnOp _ | UBinOp _ | UTupleIntro _), _ ->
        ...
        with_uvar e @@ fun x -> Ret (x :: extra, [])
    | Loc (e, _), _ -> uwiden e extra
    | _, _ ->
  ...
  let lwiden (outputs1 : lvars) (outputs2 : lvars) (e : expr) : expr =
    ...
    if outputs1 = outputs2 then e
    else Let ([], map unknown outputs1, e, Ret ([], outputs2))
end
...
module Contexts : sig
  ...
  let rec bindings c (k : ubindings) : ubindings =
    match c with
    | Leaf { ubs; _ } -> ubs @ k
    | Cat (c1, c2) -> bindings c1 (bindings c2 k)

  let bindings c = bindings c []
  let empty = Leaf { ubs = []; fill = (fun e -> e) }
  ...
  let rec fill c e =
    match c with
    | Leaf { ubs = _; fill } -> fill e
    | Cat (c1, c2) -> fill c1 (fill c2 e)
...
module Renamings = struct
  ...
  type rho = uvar UVarMap.t * lvar LVarMap.t

  (* The identity renaming. *)

  let identity : rho = (UVarMap.empty, LVarMap.empty)
  ...
  let extend ((urho, lrho) : rho) ubs uys lbs lys =
    let uxs = map fst ubs and lxs = map fst lbs in
  ...
  let compose rho ubs uys lbs lys =
    extend rho ubs (map (uapply rho) uys) lbs (map (lapply rho) lys)
...
let rec annotate (e : expr) : expr =
  ...
  | Let (ubs, lbs, e1, e2) ->
      ...
      Let (ubs, lbs, e1', e2')
  | UTupleElim (ubs, x, e) -> UTupleElim (ubs, x, annotate e)
  | LTupleElim (lbs, l, e) -> LTupleElim (lbs, l, annotate e)
