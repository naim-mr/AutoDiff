...
let eval_unop uop = match uop with OpSin -> sin | OpCos -> cos | OpExp -> exp
...
let as_real range v =
  ...
  | VReal c -> c
  | VTuple vs ->
      fail range "Expected a real; got a tuple of arity %d" (length vs)

let as_tuple range v =
  match v with
  | VTuple vs -> vs
  | _ -> fail range "Expected a tuple; got a real"

let rec print_value v =
  match v with
  | VReal c -> to_string c
  | VTuple vs -> sprintf "[%s]" (String.concat ", " (List.map print_value vs))
...
type fenv = (bindings * expr) Env.env
...
let bind_many range kind env xs vs =
  try Env.bind_many env xs vs
  with Invalid_argument _ ->
    fail range "Expected %s of arity %d; got %s of arity %d" kind (length xs)
      kind (length vs)
...
let rec eval range (fenv : fenv) (env : env) (e : expr) : value =
  match e with
  | Loc (e, range) -> eval range fenv env e
  | Var x -> lookup range "variable" env x
  | Literal c -> real c
  | UnOp (uop, e) ->
  ...
  | TupleIntro es -> VTuple (List.map (eval range fenv env) es)
  | TupleElim (xs, e1, e2) ->
...
let eval_decls (decls : decls) : fenv = List.fold_left eval_decl empty decls

let eval (prog : prog) (env : env) (e : expr) : value =
  let range = dummy (* not great *) and fenv = eval_decls prog in
  eval range fenv env e
