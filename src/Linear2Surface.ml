...
let translate_var (Linear.U x) = x
let translate_vars uxs = List.map translate_var uxs
let translate_binding (ux, _ty) = translate_var ux
let translate_bindings ubs = List.map translate_binding ubs
let transport_binding (ux, ty) = (translate_var ux, ty)
let transport_bindings ubs = List.map transport_binding ubs
...
let rec translate (e : Linear.expr) : expr =
  ...
  | Linear.ULiteral c -> Literal c
  | Linear.UUnOp (op, x) -> UnOp (op, var (translate_var x))
  | Linear.UBinOp (x1, op, x2) ->
      BinOp (var (translate_var x1), op, var (translate_var x2))
  | Linear.LZero _ | Linear.LAdd _ | Linear.LMul _ | Linear.Drop _
  | Linear.Dup _ | Linear.LTupleIntro _ | Linear.LTupleElim _ ->
      assert false
  | Linear.UTupleIntro uxs -> TupleIntro (vars (translate_vars uxs))
  ...
  | Linear.Loc (e, loc) -> (
      match loc with
      | Linear.Virtual _ -> translate e
      | Linear.Source range -> Loc (translate e, range))
...
let translate decls = List.map translate_decl decls
