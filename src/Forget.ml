...
let transform_var (L x : lvar) : uvar = U x
let transform_binding ((x, ty) : lbinding) : ubinding = (transform_var x, ty)
let transform_vars = map transform_var
let transform_bindings = map transform_binding
...
let rec uzero (ty : ty) : expr =
  ...
  | TReal -> ULiteral Real.zero
  | TTuple tys -> with_uvars (map uzero tys) @@ fun uxs -> UTupleIntro uxs
...
type env = UVarSet.t * UVarSet.t

let empty : env = UVarSet.(empty, empty)
...
let rec transform_expr (e : expr) : expr =
  match e with
  | Loc (e, range) -> Loc (transform_expr e, range)
  | Ret (uxs, lxs) -> Ret (uxs @ transform_vars lxs, [])
  | Let (ubs, lbs, e1, e2) ->
      Let
        (ubs @ transform_bindings lbs, [], transform_expr e1, transform_expr e2)
  | ULiteral _ | UUnOp _ | UBinOp _ | UTupleIntro _ -> e
  | LZero ty -> uzero ty
  | LAdd (x1, x2) -> UBinOp (transform_var x1, OpAdd, transform_var x2)
  | LMul (x1, x2) -> UBinOp (x1, OpMul, transform_var x2)
  | Drop _x -> Ret ([], [])
  | Dup x ->
      let x = transform_var x in
      Ret ([ x; x ], [])
  | UTupleElim (uxs, x1, e2) -> UTupleElim (uxs, x1, transform_expr e2)
  | LTupleIntro lxs -> UTupleIntro (transform_vars lxs)
  | LTupleElim (lbs, x1, e2) ->
      UTupleElim (transform_bindings lbs, transform_var x1, transform_expr e2)
  | FunCall (f, uxs, lxs) -> FunCall (f, uxs @ transform_vars lxs, [])
